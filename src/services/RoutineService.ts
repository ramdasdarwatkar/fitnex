...
export const RoutineService = {
  ...
  async getRoutinesWithMeta(): Promise<EnrichedRoutine[]> {
    ...
    return routines.map((route) => {
      // Find exercises and sort them by exercise_order
      const linkedExercises = rEx
        .filter((re) => re.routine_id === route.id)
      ...
      return {
        ...
        exercises: linkedExercises, // Attaching exercises to the object
      } as EnrichedRoutine;
    });
  },
